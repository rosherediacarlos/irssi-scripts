...
sub hack_command {
    ...
    if ($msg =~ /^!hack\s+(\w+)/i) {
        ...
        $server->command("msg $channel $hack_nick, elige una opción: vpn, antivirus, firewall.");
    }
}
    ...
    if (exists $pending_hacks{$nick}) {
        ...
        if (exists $valid_options{$message}) {
            ...
            if ($correct_defense =~ $message) {
                $server->command("msg $channel $nick ha podido defenderse del hackeo mediante: $message");
            } else {
                ...
                $server->command("msg $channel $nick No has podido defenderse del hackeo mediante: $message");
